...
Sends bounded random pipelines to **`$rustSdkE2e`** (same stack as e2e). Not LLVM libFuzzer; uses **`maxTime`** per aggregate and alternates empty vs non-empty collections.
...
Slow; exercises mocked integration tests in the **Rust SDK for MongoDB Extensions**, not **`proptest`**:
