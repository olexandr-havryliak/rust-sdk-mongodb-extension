...
COMPOSE=(docker compose -f examples/data-federation/docker-compose.yml --project-name data-federation-example)

usage() {
  cat <<'EOF'
Usage: ./examples/data-federation/run-demo.sh [COMMAND]
...
case "$cmd" in
  -h|--help|help)
    ...
    ;;
  up|start)
    echo "==> Building data-federation example image (MongoDB ${MONGO_IMAGE})..."
    "${COMPOSE[@]}" build
    echo "==> Starting mongod (containers stay up; use '$0 down' to remove)..."
    "${COMPOSE[@]}" up -d
    wait_for_mongo
    echo "==> Ready. Connect:"
    echo "    mongosh --port 27022"
    echo "  Example:"
    echo "    use data_federation_demo; db.createCollection(\"n\"); db.n.aggregate([{ \\\$readLocalJsonl: { path: \"sample.ndjson\" } }])"
    echo "  Stop:"
    echo "    $0 down"
    exit 0
    ;;
  ...
  run)
    ...
    echo "==> Running data-federation demo script..."
    set +e
    ...
    echo "==> Demo passed. Try: mongosh --port 27022"
    echo "    use data_federation_demo; db.createCollection(\"n\"); db.n.aggregate([{ \\\$readLocalJsonl: { path: \"sample.ndjson\" } }])"
    exit 0
    ;;
  *)
